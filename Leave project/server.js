...
function createDatabasePool(useDatabase = true) {
    ...
    const user = envValue("MYSQLUSER", "MYSQL_USER", "DB_USER") || "root";
    const password = envValue("MYSQLPASSWORD", "MYSQL_PASSWORD", "DB_PASSWORD") || "amansaini@123";
    const database = envValue("MYSQLDATABASE", "MYSQL_DATABASE", "DB_NAME") || "leaveDB";
    const port = Number(envValue("MYSQLPORT", "MYSQL_PORT", "DB_PORT")) || 3306;
...
async function ensureDatabaseExists() {
    const database = envValue("MYSQLDATABASE", "MYSQL_DATABASE", "DB_NAME") || "leaveDB";
    const tempPool = createDatabasePool(false);
    try {
...
async function startServer() {
    ...
    try {
        ...
        console.log("Server is still running to serve static files, but database features will fail until MySQL is started.");
    }
}
